...
const OrdenesDetallePaqTable = ({
  
}) => {
  const [loadingTable, setLoadingTable] = useState(true);
  const [OrdenesDetalleData, setOrdenesDetalleData] = useState([]);
  const [pedidosDetalleEstatusPaq, setPedidosDetalleEstatusPaq] = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setOrdenesDetalleData(selectedOrdenesData.ordenes_presenta_ps_paq); //Se ponen los datos en el useState pero solo los del subdocumento info_ad
...
export default OrdenesDetallePaqTable;
