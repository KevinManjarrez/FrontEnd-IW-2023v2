...
//HELPERS
//import { OrdenesProveedorValues } from "../../helpers/OrdenesProveedorValues";
...
const OrdenesProveedorModal = ({ OrdenesProveedorShowModal, setOrdenesProveedorShowModal,row,handleReload }) => {
    ...
    const formik = useFormik({
        initialValues: {
            IdOrdenOK: "",
            IdOrdenBK: "342001",
            Observacion: "IdTipoOrden-IdVenta"
        },
        validationSchema: Yup.object({
            IdOrdenOK: Yup.string().required("Campo requerido"),
            IdOrdenBK: Yup.string().required("Campo requerido").max(1, 'Solo se permite una letra').matches(/^[SN]$/, 'Solo se permite un caracter S/N'),
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                for (let index = 0; index < ordenExistente.ordenes_proveedor.length; index++) {
                    console.log("Entro")
                    ordenExistente.ordenes_estatus[index]= {
                        IdTipoEstatusOK: ordenExistente.ordenes_proveedor[index].IdTipoEstatusOK,
                        Actual: "N",
                        Observacion:ordenExistente.ordenes_proveedor[index].Observacion
                      };
                      console.log("Realizo",ordenExistente)
                }
                const EstatusOrdenes = OrdenesProveedorValues(values, ordenExistente);
    ...
    return(
        <Dialog
            open={OrdenesProveedorShowModal}
            onClose={() => setOrdenesProveedorShowModal(false)}
            fullWidth
        >
    ...
    />*/}
                ...
                >
                    ...
                    <LoadingButton
                        ...
                        startIcon={<CloseIcon />}
                        variant="outlined"
                        onClick={() => setOrdenesProveedorShowModal(false)}
                    >
                        <span>CERRAR</span>
                    </LoadingButton>
